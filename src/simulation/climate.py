...
#
# P5 INTEGRATION PATCH (perf) — original version scanned every cell of the
# world for EVERY cell, making one simulation year take ~1.2 seconds, which
# is too slow for the 1x/2x/4x game speeds. This version precomputes one
# distance-to-water field per year with a multi-source BFS (grid distance
# instead of exact euclidean — same MAX_DISTANCE falloff). Same signature,
# same 0..1 return. ~100x faster. Original scan kept below for reference.
# — Winston (P5). P2 please review.
_water_field = {"year": None, "world_id": None, "field": None}


def _compute_water_distance_field(world):
    from collections import deque
    INF = float("inf")
    dist = [[INF] * world.width for _ in range(world.height)]
    queue = deque()
    for row in world.grid:
        for cell in row:
            if cell.biome == WATER:
                dist[cell.y][cell.x] = 0.0
                queue.append((cell.x, cell.y))
    while queue:
        x, y = queue.popleft()
        d = dist[y][x] + 1.0
        for nx, ny in ((x+1, y), (x-1, y), (x, y+1), (x, y-1)):
            if 0 <= nx < world.width and 0 <= ny < world.height and dist[ny][nx] > d:
                dist[ny][nx] = d
                queue.append((nx, ny))
    return dist
...
def proximity_to_water(target_cell, world):
    if(target_cell.biome == WATER):
        return 1
    key = (world.year, id(world))
    if (_water_field["year"], _water_field["world_id"]) != key:
        _water_field["field"] = _compute_water_distance_field(world)
        _water_field["year"], _water_field["world_id"] = key
    smallest_distance = _water_field["field"][target_cell.y][target_cell.x]
...
def _original_proximity_to_water(target_cell, world):
    if(target_cell.biome == WATER):
        return 1
    x1 = target_cell.x
    y1 = target_cell.y
